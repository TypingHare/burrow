...
type Builder struct {
	...
	// CartonDefs is a slice of carton definitions to build the executable.
...
func (b *Builder) GenerateMagicGoModFile() error {
	// Check if Go mod file exists in the Burrow source directory.
	...
	for _, cartonDef := range b.CartonDefs {
		localPathsByCartonNames[cartonDef.Name] = cartonDef.Path
	}

	majorVersion := share.GetBurrowMajorVersion()
	...
	// dependency.
	cartonNames := CartonNames(b.CartonDefs)
	for _, cartonName := range cartonNames {
		...
		if !isLocalCarton || localCartonPath == "" {
			...
			// Module path should be in the form of
			// "<cartonName>/v<majorVersion>" to be compatible with Go's module
			// versioning.
	...
	// Run "go mod download" to download the dependencies specified in the magic
	// Go mod file.
...
// BuildBurrow builds Burrow using the generated module file and magic source.
func (b *Builder) BuildBurrow() error {
