...
func InstallCommand(decor share.IDecor) *cobra.Command {
	var path string
...
This command adds a carton to the current clutter decor spec.
...
instead of downloading the carton from its normal source location.
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			cartonName := args[0]
			return share.InstallCarton(decor, cartonName, path)
		},
	}
