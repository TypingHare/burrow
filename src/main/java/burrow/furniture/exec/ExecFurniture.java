...
public class ExecFurniture extends Furniture {
    ...
    public void execute(
        ...
        final var shell = context.getConfig().get(ConfigKey.EXEC_SHELL);
        final var processBuilder = new ProcessBuilder(shell, "-c", '"' + command + '"');

        // Set the working directory
        final var workingDirectory = commandContext.getEnvironment().getWorkingDirectory();
        ...
        commandContext.getBuffer().append(exitCode == 0 ? stringBuilder : errorOutput);
        commandContext.set(CommandContext.Key.EXIT_CODE, exitCode);
    }
